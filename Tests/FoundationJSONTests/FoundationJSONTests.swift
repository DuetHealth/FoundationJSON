...
class TestFoundationJSONEncoder : XCTestCase {

    // MARK: - Non-Foundation Additions
    func testPossiblyDecodable() throws {
        ...
        var object = try FoundationJSONDecoder().decode(Test.self, from: noData)
        XCTAssertEqual(object.value, nil)

        object = try FoundationJSONDecoder().decode(Test.self, from: stringData)
        XCTAssertEqual(object.value, nil)

        object = try FoundationJSONDecoder().decode(Test.self, from: intData)
        XCTAssertEqual(object.value, nil)

        MyFlag.shouldDecode = true

        object = try FoundationJSONDecoder().decode(Test.self, from: noData)
        XCTAssertEqual(object.value, nil)

        XCTAssertThrowsError(try FoundationJSONDecoder().decode(Test.self, from: stringData))
    ...
    func test_encodingTopLevelFragments() {
        ...
        func _testFragment<T: Codable & Equatable>(value: T, fragment: String) {
            ...
            do {
                data = try FoundationJSONEncoder().encode(value)
            ...
            do {
                let decodedValue = try FoundationJSONDecoder().decode(T.self, from: data)
    ...
    func test_encodingOutputFormattingPrettyPrinted() throws {
        ...
        let encoder = FoundationJSONEncoder()
...
""")
    ...
    func test_encodingNonConformingFloatStrings() {
        let encodingStrategy: FoundationJSONEncoder.NonConformingFloatEncodingStrategy = .convertToString(positiveInfinity: "INF", negativeInfinity: "-INF", nan: "NaN")
        let decodingStrategy: FoundationJSONDecoder.NonConformingFloatDecodingStrategy = .convertFromString(positiveInfinity: "INF", negativeInfinity: "-INF", nan: "NaN")
    ...
    func test_codingOfBool() {
        ...
        do {
            _ = try FoundationJSONDecoder().decode([Bool].self, from: "[1]".data(using: .utf8)!)
        ...
        func testValue(_ valueName: String) {
            do {
                ...
                _ = try FoundationJSONDecoder().decode(Foo.self, from: jsonData)
        ...
        if let falseFoo = try? FoundationJSONDecoder().decode(Foo.self, from: falseJsonData) {
        ...
        if let trueFoo = try? FoundationJSONDecoder().decode(Foo.self, from: trueJsonData) {
    ...
    func test_codingOfFloat() {
        ...
        XCTAssertThrowsError(try FoundationJSONDecoder().decode(Float.self, from: "1e100".data(using: .utf8)!))
    }
    ...
    func test_codingOfDouble() {
        ...
        // Check value too large fails to decode.
        XCTAssertThrowsError(try FoundationJSONDecoder().decode(Double.self, from: "100e323".data(using: .utf8)!))
    }
    ...
    func test_numericLimits() {
        ...
        func decode(_ type: String, _ value: String) throws {
            ...
            key.append("Value")
            _ = try FoundationJSONDecoder().decode(DataStruct.self, from: "{ \"\(key)\": \(value) }".data(using: .utf8)!)
        }
    ...
    func test_snake_case_encoding() throws {
        ...
        let decoder = FoundationJSONDecoder()
    ...
    func test_OutputFormattingValues() {
        XCTAssertEqual(FoundationJSONEncoder.OutputFormatting.prettyPrinted.rawValue, 1)
        if #available(OSX 10.13, *), #available(iOS 11.0, *) {
            XCTAssertEqual(FoundationJSONEncoder.OutputFormatting.sortedKeys.rawValue, 2)
        }
        if #available(iOS 13.0, *) {
            XCTAssertEqual(FoundationJSONEncoder.OutputFormatting.withoutEscapingSlashes.rawValue, 8)
        }
    }
    ...
    private func _testEncodeFailure<T : Encodable>(of value: T) {
        do {
            let _ = try FoundationJSONEncoder().encode(value)
    ...
    private func _testRoundTrip<T>(of value: T,
                                   ...
                                   outputFormatting: FoundationJSONEncoder.OutputFormatting = [],
                                   dateEncodingStrategy: FoundationJSONEncoder.DateEncodingStrategy = .deferredToDate,
                                   dateDecodingStrategy: FoundationJSONDecoder.DateDecodingStrategy = .deferredToDate,
                                   dataEncodingStrategy: FoundationJSONEncoder.DataEncodingStrategy = .base64,
                                   dataDecodingStrategy: FoundationJSONDecoder.DataDecodingStrategy = .base64,
                                   nonConformingFloatEncodingStrategy: FoundationJSONEncoder.NonConformingFloatEncodingStrategy = .throw,
                                   nonConformingFloatDecodingStrategy: FoundationJSONDecoder.NonConformingFloatDecodingStrategy = .throw) where T : Codable, T : Equatable {
...
extension TestFoundationJSONEncoder {
    static var allTests: [(String, (TestFoundationJSONEncoder) -> () throws -> Void)] {
        return [
