...
/// `FoundationJSONEncoder` facilitates the encoding of `Encodable` values into JSON.
open class FoundationJSONEncoder {
    // MARK: Options
    ...
    func encodeAsJSONValue<T: Encodable>(_ value: T) throws -> JSONValue {
        let encoder = FoundationJSONEncoderImpl(options: self.options, codingPath: [])
...
// MARK: - _FoundationJSONEncoder

private enum JSONFuture {
    case value(JSONValue)
    case encoder(FoundationJSONEncoderImpl)
    ...
    class RefArray {
        ...
        @inline(__always) func append(_ encoder: FoundationJSONEncoderImpl) {
    ...
    class RefObject {
        ...
        @inline(__always) func set(_ encoder: FoundationJSONEncoderImpl, for key: String) {
            switch self.dict[key] {
            case .encoder:
                preconditionFailure("For key \"\(key)\" an encoder has already been created.")
            case .nestedObject:
...
private class FoundationJSONEncoderImpl {
    let options: FoundationJSONEncoder._Options
    ...
    init(options: FoundationJSONEncoder._Options, codingPath: [CodingKey]) {
        self.options = options
        self.codingPath = codingPath
    }
}

extension FoundationJSONEncoderImpl: Encoder {
...
extension FoundationJSONEncoderImpl: _SpecialTreatmentEncoder {
    var impl: FoundationJSONEncoderImpl {
        return self
    }
...
private protocol _SpecialTreatmentEncoder {
    ...
    var options: FoundationJSONEncoder._Options { get }
    var impl: FoundationJSONEncoderImpl { get }
}
...
extension _SpecialTreatmentEncoder {
    ...
    func wrapObject(_ object: [String: Encodable], for additionalKey: CodingKey?) throws -> JSONValue {
        ...
        try object.forEach { (key, value) in
            ...
            let encoder = FoundationJSONEncoderImpl(options: self.options, codingPath: elemCodingPath)
    ...
    fileprivate func getEncoder(for additionalKey: CodingKey?) -> FoundationJSONEncoderImpl {
        if let additionalKey = additionalKey {
            var newCodingPath = self.codingPath
            newCodingPath.append(additionalKey)
            return FoundationJSONEncoderImpl(options: self.options, codingPath: newCodingPath)
        }
...
private struct JSONKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol, _SpecialTreatmentEncoder {
    ...
    let impl: FoundationJSONEncoderImpl
    let object: JSONFuture.RefObject
    let codingPath: [CodingKey]

    private var firstValueWritten: Bool = false
    fileprivate var options: FoundationJSONEncoder._Options {
        return self.impl.options
    }

    init(impl: FoundationJSONEncoderImpl, codingPath: [CodingKey]) {
        self.impl = impl
    ...
    init(impl: FoundationJSONEncoderImpl, object: JSONFuture.RefObject, codingPath: [CodingKey]) {
        self.impl = impl
        self.object = object
        self.codingPath = codingPath
    }
    ...
    private func _converted(_ key: Key) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = FoundationJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
...
private struct JSONUnkeyedEncodingContainer: UnkeyedEncodingContainer, _SpecialTreatmentEncoder {
    ...
    // used for nested containers
    init(impl: FoundationJSONEncoderImpl, array: JSONFuture.RefArray, codingPath: [CodingKey]) {
        self.impl = impl
        self.array = array
        self.codingPath = codingPath
    }
...
extension JSONValue {
    ...
    fileprivate struct Writer {
        let options: FoundationJSONEncoder.OutputFormatting

        init(options: FoundationJSONEncoder.OutputFormatting) {
            self.options = options
        }
...
extension EncodingError {
    ...
    fileprivate static func _invalidFloatingPointValue<T: FloatingPoint>(_ value: T, at codingPath: [CodingKey]) -> EncodingError {
        ...
        let debugDescription = "Unable to encode \(valueDescription) directly in JSON. Use FoundationJSONEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
