...
/// `FoundationJSONDecoder` facilitates the decoding of JSON into semantic `Decodable` types.
open class FoundationJSONDecoder {
    // MARK: Options
    ...
    open func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        do {
            ...
            return try FoundationJSONDecoderImpl(userInfo: self.userInfo, from: json, codingPath: [], options: self.options).unwrap(as: T.self)
...
// MARK: - _FoundationJSONDecoder

fileprivate struct FoundationJSONDecoderImpl {
    let codingPath: [CodingKey]
    let userInfo: [CodingUserInfoKey: Any]

    let json: JSONValue
    let options: FoundationJSONDecoder._Options

    init(userInfo: [CodingUserInfoKey: Any], from json: JSONValue, codingPath: [CodingKey], options: FoundationJSONDecoder._Options) {
...
extension FoundationJSONDecoderImpl: Decoder {
    ...
    private func unwrapDictionary<T: Decodable>(as: T.Type) throws -> T {
        ...
        for (key, value) in object {
            ...
            let newDecoder = FoundationJSONDecoderImpl(userInfo: self.userInfo, from: value, codingPath: newPath, options: self.options)
...
extension Decodable {
    fileprivate static func createByDirectlyUnwrapping(from decoder: FoundationJSONDecoderImpl) throws -> Self {
...
extension FoundationJSONDecoderImpl {
    struct SingleValueContainter: SingleValueDecodingContainer {
        let impl: FoundationJSONDecoderImpl
        let value: JSONValue
        let codingPath: [CodingKey]

        init(impl: FoundationJSONDecoderImpl, codingPath: [CodingKey], json: JSONValue) {
            self.impl = impl
            self.codingPath = codingPath
            self.value = json
        }
...
extension FoundationJSONDecoderImpl {
    struct KeyedContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
        ...
        init(impl: FoundationJSONDecoderImpl, codingPath: [CodingKey], dictionary: [String: JSONValue]) {
            self.impl = impl
            self.codingPath = codingPath
            ...
            case .convertFromSnakeCase:
                ...
                dictionary.forEach { (key, value) in
                    converted[FoundationJSONDecoder.KeyDecodingStrategy._convertFromSnakeCase(key)] = value
                }
        ...
        func decode<T>(_ type: T.Type, forKey key: K) throws -> T where T: Decodable {
            ...
            if dictionary[key.stringValue] == nil && skipIfNotFound {
        ...
        private func decoderForKey<LocalKey: CodingKey>(_ key: LocalKey) throws -> FoundationJSONDecoderImpl {
            let value = try getValue(forKey: key)
            var newPath = self.codingPath
            newPath.append(key)

            return FoundationJSONDecoderImpl(
...
extension FoundationJSONDecoderImpl {
    struct UnkeyedContainer: UnkeyedDecodingContainer {
        ...
        init(impl: FoundationJSONDecoderImpl, codingPath: [CodingKey], array: [JSONValue]) {
            self.impl = impl
            self.codingPath = codingPath
            self.array = array
        }
        ...
        private mutating func decoderForNextElement<T>(ofType: T.Type) throws -> FoundationJSONDecoderImpl {
